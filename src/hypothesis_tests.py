...
def plot_statistical_distribution(df, column, title_suffix=""):
    """
    Creates a three-panel visualization optimized for insurance risk
    analysis: 1. Raw Distribution: Shows skewness and central
    tendency .
    2. Log-Transformation: Normalizes skewed data
     for statistical testing.
    3. Box Plot: Explicitly identifies high-risk outliers.
    """
    ...
    ax1.axvline(
        mu, color="navy", linestyle="--", linewidth=2,
        label=f"Mean: {mu:.1f}"
    )
    ax1.axvline(
        median,
    ...
    ax2.plot(
        x, norm.pdf(x, l_mu, l_std), color="navy",
        linewidth=2, label="Normal Fit"
    )

    # Add Mean and Median lines for log-transformed data
    ax2.axvline(
        l_mu, color="red", linestyle="--", linewidth=2,
        label=f"Mean: {l_mu:.2f}"
    )
    ax2.axvline(
        l_median, color="green", linestyle="-", linewidth=2,
        label=f"Median: {l_median:.2f}",
    )

    ax2.set_title(
        f"Log({column}) — Normalized for Testing",
    ...
    ax3.set_title(
        f"{column} — Outlier Detection", fontsize=13,
        fontweight="bold"
    )
    ...
    pull_dir = 'Pulled RIGHT' if skew > 0 else 'Pulled LEFT'
    print(f"Mean:     {mu:.2f}  ← {pull_dir} (Slide 6)")
    print(
        f'Median:   {median:.2f}  ← More representative of'
        f' "Typical" risk'
    )
    skew_type = 'High Skew' if abs(skew) > 1 else 'Moderate Skew'
    print(f"Skewness: {skew:.3f}  ({skew_type})")
    outlier_count = len(outliers)
    print(
        f"Outliers: {outlier_count} events detected above"
        f" {outlier_threshold:.2f} (IQR Method)"
    )
    print(f"Max Risk: {df[column].max():.2f} (Highest single value)")

    if abs(skew) > 1:
        print(
            "\nADVICE: Use Log-transformed data for"
            " T-tests/ANOVA to satisfy Normality assumptions."
        )


def plot_poisson_distribution(df, column, group_col=None, title_suffix=""):
    """
    Three-panel Poisson analysis for Claim Frequency:
    1. Observed vs. Theoretical: Checks if the data follows Poisson logic.
    2. Log-Scale Tail: Visualizes rare multi-claim events.
    3. Group Comparison: Visualizes frequency differences between
    segments (A/B testing).
    """
    # 1. Overall Stats
    ...
    width = 0.35
    ax1.bar(
        k_values - width / 2, obs_data, width=width,
        color="steelblue", alpha=0.7,
        label="Observed Risk"
    )
    ax1.bar(
        k_values + width / 2, theory_probs, width=width,
        color="coral", alpha=0.6,
        label=f"Poisson(λ={lam_total:.4f})"
    )

    ax1.set_title(
        "Observed vs. Theoretical Poisson",
        fontsize=13, fontweight="bold"
    )
    ...
    ax2 = axes[1]
    ax2.bar(
        k_values, obs_data, color="steelblue", alpha=0.7,
        label="Observed"
    )
    ax2.plot(
        k_values, theory_probs, "o-", color="navy", markersize=8,
        label="Poisson Fit"
    )

    ax2.set_yscale("log")
    ax2.set_title(
        "Tail Behavior (Log Scale)", fontsize=13, fontweight="bold"
    )
    ...
    if group_col and df[group_col].nunique() >= 2:
        ...
        for i, g in enumerate(groups):
            ...
            g_probs = [
                sum(group_data == k) / len(group_data) for k in k_values
            ]
        ...
        ax3.set_title(
            f"Frequency by {group_col}", fontsize=13, fontweight="bold"
        )
    else:
        ...
        for lam, c in [
            ...
            ax3.plot(
                k_range,
                stats.poisson.pmf(k_range, mu=lam),
                "o-",
                color=c,
                alpha=0.6,
                label=f"λ={lam:.2f}",
        ...
        ax3.set_title(
            "Theoretical λ Comparison", fontsize=13, fontweight="bold"
        )
    ...
    print(
        f"Ratio (Var/Mean):           {variance / lam_total:.3f}"
        f" (Ideally ~1 for Poisson)"
    )

    if group_col:
        for g in df[group_col].unique()[:2]:
            g_mean = df[df[group_col] == g][column].mean()
            print(f"Group {g} Frequency (λ): {g_mean:.5f}")

    print(
        "\nBusiness Insight: Higher λ indicates a segment with"
        " higher service costs and risk frequency (Slide 8)."
    )
...
def interpret_decision(p_value: float, alpha: float = 0.05) -> bool:
    ...
    if p_value < alpha:
        ...
        print(
            f'DECISION: p = {p_value:.4f} ≥ α ({alpha}) →'
            f' FAIL TO REJECT H₀'
        )
        return False
...
def plot_chi_squared(
    contingency_table: pd.DataFrame, p_value: float, title: str
):
    plt.figure(figsize=(10, 6))
    sns.heatmap(
        contingency_table, annot=True, fmt='d', cmap='Blues', cbar=False
    )
    plt.title(
...
def plot_numerical_comparison(
    data_a, data_b, label_a: str, label_b: str,
    p_value: float, title: str
):
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))

    # Histogram
    axes[0].hist(
        data_a, bins=50, alpha=0.7, color='coral', density=True,
        label=label_a
    )
    axes[0].hist(
        data_b, bins=50, alpha=0.7, color='steelblue', density=True,
        label=label_b
    )
    axes[0].axvline(
        data_a.mean(), color='darkred', linestyle='--',
        linewidth=2, label=f'{label_a} Mean'
    )
    axes[0].axvline(
        data_b.mean(), color='navy', linestyle='--', linewidth=2,
    ...
    # Boxplot
    axes[1].boxplot(
        [data_a, data_b], labels=[label_a, label_b], patch_artist=True
    )
...
def chi_squared_claim_frequency_test(
    df: pd.DataFrame,
    group_col: str,
    ...
    """Chi-Squared Test with optional category filtering."""
    ...
    if contingency.shape[0] < 2:
        print(
            "❌ Not enough groups for Chi-Squared test after "
            "filtering."
        )
    ...
    print(
        f'H₀: No risk difference in Claim Frequency '
        f'across {group_col}'
    )
    ...
    plot_title = title or f"Claim Frequency by {group_col}"
    plot_chi_squared(contingency, p_value, plot_title)
...
def t_test(
    ...
    print(
        f"Sample Size - {group_a}: {len(data_a)} | "
        f"{group_b}: {len(data_b)}"
    )

    if len(data_a) < min_sample or len(data_b) < min_sample:
        print(
            f"⚠️ Warning: One or both groups have small sample size "
            f"(< {min_sample})"
        )
    ...
    print(
        f'H₀: Mean {value_col} is the same for {group_a} and '
        f'{group_b}'
    )
    ...
    if rejected and not pd.isna(p_value):
        if data_a.mean() != 0:
            diff_pct = (data_b.mean() - data_a.mean()) / (
                data_a.mean()
            ) * 100
        else:
            diff_pct = np.nan
        ...
        print(
            f"→ {group_b} has {diff_pct:+.1f}% {direction} "
            f"average {value_col}"
        )

    plot_title = title or f"{value_col} by {group_col}"
    plot_numerical_comparison(
        data_a, data_b, str(group_a), str(group_b), p_value, plot_title
    )
...
def anova_test(
    ...
    valid_groups = (
        group_sizes[group_sizes >= min_count]
    ...
    print(
        f"Testing {len(valid_groups)} largest groups "
        f"(min {min_count} samples each)"
    )

    groups = [
        group_data[value_col].dropna()
        for _, group_data in filtered_df.groupby(
            group_col
        )
    ]
    ...
    plt.title(
        f'{title or f"{value_col} by {group_col}"}\nANOVA p = {p_value:.4f}',
        fontweight='bold')
